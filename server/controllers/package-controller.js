...
const getPackagesUploadedByOthers = async (req, res, next) => {
  try {
    ...
    const otherPackages = packages.map((package) => {
      const createdAt = new Date(package.createdAt);
    ...
    const otherPackagesUploadedBy = otherPackages.filter((package) => {
      return package.user && package.user.id !== loggedInUserId;
    });
...
const getAllBookmarkedPackages = async (req, res) => {
  try {
    ...
    const bookmarkedPackages = user.bookmarkedPackages.map((package) => {
      const createdAt = new Date(package.createdAt);
      const currentTime = new Date();
      const timeDifference = Math.abs(currentTime - createdAt);
      ...
      return {
        ...package.toObject(),
        formattedCreatedAt: formattedTime,
        isBookmarked: true, // Since it's a bookmarked package, set isBookmarked to true
      };
    });
...
const uploadPackageCover = async (req, res, next) => {
  try {
    ...
    // Save the package cover image and get the filename
...
const addReviewToPackage = async (req, res, next) => {
  ...
  try {
    ...
    const existingReview = package.reviews_and_ratings.find(
    ...
    // Add the new review to the package
    package.reviews_and_ratings.push({
...
const getPackageReviews = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ reviews: package.reviews_and_ratings });
  } catch (error) {
    next(error);
  }
};
