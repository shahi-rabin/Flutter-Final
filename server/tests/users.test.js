...
test("get all exchange requests for a user", async () => {
  // Create a test user
  const newUser = await User.create({
    username: "exchangeUser",
    password: "test123456789",
    fullname: "Exchange User",
    email: "exchange@test.com",
  });

  // Create a test exchange request for the user
  const exchangeRequest = await ExchangeRequest.create({
    requester: newUser._id,
    requestedUser: newUser._id,
    requestedBook: "5f9d7b7b9d3b9b2d0c9b0b0b",
    proposalBook: "5f9d7b7b9d3b9b2d0c9b0b0b",
    status: "pending",
    message: "Test exchange request",
  });

  const res = await api
    .get(`/users/${newUser._id}/exchange-requests`)
    .set("Authorization", `Bearer ${validToken}`)
    .expect(200);
});
